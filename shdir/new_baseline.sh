...
for model in $model_list; do
    for dataset in $dataset_list; do
        for output_len in $out_list; do
            ...
            exec="torchrun --nproc_per_node=$dev_num --nnodes=1 informermain.py $flags -C $cuda_device -e $epoch -o $output_len -b $batch_size --fixed_seed 3407 -m $model -d $dataset -S save/$prefix/$exec_date/$model/$dataset/$output_len"
            echo "$exec"
