...
for model in informer autoformer fedformer pyraformer transformer reformer; do
  for dataset in gweather etth1 etth2 ettm1 ettm2 exchange ill; do
    for output_len in 24 48 96 192 360 720; do
      ...
      echo torchrun --nproc_per_node=$dev_num --nnodes=1 informermain.py -GBDMC $cuda_device -e $epoch -o $output_len -b $batch_size --fixed_seed 3407 -m $model -d $dataset -S save/23.6.13/$model/$dataset/$output_len --titan
      torchrun --nproc_per_node=$dev_num --nnodes=1 informermain.py -GBDMC $cuda_device -e $epoch -o $output_len -b $batch_size --fixed_seed 3407 -m $model -d $dataset -S save/23.6.13/$model/$dataset/$output_len --titan
    done
  done
done
