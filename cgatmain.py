...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lr', type=float, default=.0001)
    ...
    if platform.system() == 'Windows':
        ...
        if args.fudan:
            ...
            map_root = '/remote-home/liuwenbo/pycproj/dataset/map'
        elif args.titan:
    ...
    if (multiGPU and local_rank == 0) or not multiGPU:
        if test_set_to_validate:
            test_loader = DataLoader(InformerDataset(valid_input, valid_gt, valid_encoding), batch_size=batch_size,
                                     shuffle=False)
        else:
            test_loader = DataLoader(InformerDataset(test_input, test_gt, test_encoding), batch_size=batch_size,
                                     shuffle=False)

    model = CGAT(input_len, output_len, num_sensors, d_model=d_model, n_heads=n_heads, individual=individual)
    model.to(device)
