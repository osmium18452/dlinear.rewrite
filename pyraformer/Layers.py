...
class Decoder(nn.Module):
    ...
    def __init__(self, mask, enc_in,
                 d_inner_hid=512, n_head=8, d_k=128, d_v=128, dropout=.05, embed_type='CustomEmbedding', d_model=256,
                 covariate_size=4, seq_num=1):
        super().__init__()

        self.mask = mask

        self.layers = nn.ModuleList([
            DecoderLayer(d_model, d_inner_hid, n_head, d_k, d_v, dropout=dropout, normalize_before=False),
            DecoderLayer(d_model, d_inner_hid, n_head, d_k, d_v, dropout=dropout, normalize_before=False)
        ])
